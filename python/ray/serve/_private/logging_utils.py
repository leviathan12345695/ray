...
class IdleTimeoutMemoryHandler(logging.handlers.MemoryHandler):
    ...
    This preserves the standard MemoryHandler flush triggers, while adding an
    opt-in timer for sparse traffic where buffered INFO logs may otherwise stay
...
def configure_component_logger(
    *,
    ...
    if RAY_SERVE_REQUEST_PATH_LOG_FLUSH_TIMEOUT_S > 0 and buffer_size > 1:
